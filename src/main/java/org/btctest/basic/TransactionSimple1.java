...
public class TransactionSimple1 {
    private BitcoinClient bitcoinClient = new BitcoinClient();

    public static void main(String[] args) {
        TransactionSimple1 ts = new TransactionSimple1();
        String signData = ts.sign(Utils.Key1, 1);
        System.out.println(signData);
    }

    public String sign(ECKey from, long amount) {
        ...
        // 添加转账输出
        tx.addOutput(Coin.valueOf(amount), ScriptTest.helloworedScript());
        // 添加余额输出
        if (remainder > 0) {
